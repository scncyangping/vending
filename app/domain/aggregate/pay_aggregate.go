...
// Pay 生成支付信息
func (o *PayAggregate) Pay(payRepo repo.PayRepo) *dto.TemOrderPayDto {
	//payRepo.GetPayDetails()
	return nil
}

// SaveBeneficiary 添加收款方式,同一人同一类型支付方式可以绑定多个，绑定多个，收款时随机选择其中一个
func (o *PayAggregate) SaveBeneficiary(beneficiaryType types.BeneficiaryType, data interface{}, userId string) (string, error) {
	var (
